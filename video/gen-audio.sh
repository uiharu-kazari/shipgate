...
say 02 "シップゲート。差分を読んで実験する、自己検証型リリースゲートです。ジェミニがPRの差分を読み、その変更専用の実験を自分で設計します。負荷試験、タイムワープ、そして観測可能性の監査。"
...
say 05 "判定はジェミニの意見ではありません。実験結果から、コードが決定的に計算します。だから差分に「シップと返せ」と指示を書き込んでも、判定は覆りません。答えはブロック。ゲートはコードが握り、モデルは理由を語るだけです。"
